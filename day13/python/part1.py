...
class Amplifier:
    def __init__(self, program: Tuple[int, ...], prg_input: int) -> None:
        ...
        self.prg_output: List[int] = []         # collect all outputs
    ...
    def run(self) -> None:
        ...
        while inst.opcode != 99:
            if inst.opcode == 1:    # add
                ...
                self.prg_output.append(value)      # save last output
...
def process(data: List[int]) -> None:
    triplets = helper.grouper(data, 3)
    result = sum(1 for triplet in triplets if triplet[-1] == 2)
    #
    print(result)
...
def main() -> None:
    ...
    ampl = Amplifier(program, 2)    # test mode
    ampl.run()
    process(ampl.get_output())
